...
const getProblemIcon = (severity: Severity): React.ReactElement => {
  switch (severity) {
    case Severity.ERROR:
      return <XmarkCircle className="text-[#f76e6e]" />
    case Severity.WARNING:
      return <WarningTriangle className="text-[#E29B36]" />
    case Severity.INFO:
      return <InfoCircle />
  }
}
...
const ProblemsComponent = {
  Root: function Root({ children }: { children: React.ReactNode }) {
    return <div className="text-[13px] leading-[15px] min-h-12 bg-[#070709] text-[#c9c9cf] ">{children}</div>
  },
