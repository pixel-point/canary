...
import {
  dedupEdges,
  getEdgesForChildNodes
} from "../../../../../components/Canvas/utils/EdgeUtils";
...
export default function StageNode(props: NodeProps<GroupNodeProps>) {
  const { nodes, edges, deleteElements, addEdges, updateNodes } =
    useFlowStore();
  ...
  useEffect(() => {
    ...
    const layoutedElements = performLayout({
      nodes: [
        parallel
          ? updateNodePositionType(parentNode, PositionType.ABSOLUTE)
          : parentNode,
        ...childNodes
      ],
  ...
  const shouldUpdateChildNode = (parentNodeId: string, childNode: Node) => {
    return (
      ...
      [NodeType.ATOMIC, NodeType.ANCHOR].includes(childNode.type as NodeType)
    );
  };

  const orientation = memberNodes.every(
    (node) => (node.data as DefaultNodeProps)?.parallel
  )
    ? GroupOrientation.TB
    : GroupOrientation.LR;
  ...
  const childrenCount = useMemo(
    (): number => getChildNodes(nodeId, nodes).length,
    [nodeId, nodes]
  );
      ...
      >
        <div className={css.tools}>
          <div className={css.header}>
            <div style={{ display: "flex", alignItems: "center" }}>
              ...
              {childrenCount > 0 && (
                <span className={css.count}>&nbsp;({childrenCount})</span>
              )}
            </div>
          </div>
          {/* <Menubar
        ...
        {expanded && orientation === GroupOrientation.TB && (
          <Plus
            color="white"
            onClick={() => {}}
            className={cx(css.icon, css.plusIcon)}
          />
        )}
      </div>
      {/**
