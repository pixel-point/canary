// This file is automatically generated. Modifying its content manually is discouraged.
import { registerIcon } from '@harnessio/svg-icon'
import type { IconProps } from '@harnessio/svg-icon-react'
import { Icon } from '@harnessio/svg-icon-react'

const name = 'number-5-square-solid/noir'

registerIcon(
  name,
  `<svg width="24" height="24" fill="none" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" d="M3.6 2.25A1.35 1.35 0 0 0 2.25 3.6v16.8c0 .746.604 1.35 1.35 1.35h16.8a1.35 1.35 0 0 0 1.35-1.35V3.6a1.35 1.35 0 0 0-1.35-1.35H3.6ZM13.75 13.5c0 1.062-.76 1.75-1.75 1.75-1.131 0-1.75-.707-1.75-1.25a.75.75 0 0 0-1.5 0c0 1.666 1.62 2.75 3.25 2.75 1.771 0 3.25-1.312 3.25-3.25 0-.836-.17-1.645-.632-2.262-.488-.652-1.225-.988-2.118-.988h-2.115l.25-1.5H14a.75.75 0 0 0 0-1.5h-4a.75.75 0 0 0-.74.627l-.5 3a.75.75 0 0 0 .74.873h3c.488 0 .75.164.917.387.193.258.333.7.333 1.363Z" clip-rule="evenodd"/></svg>`
)

export function Number5SquareSolid(props: IconProps) {
  return <Icon name={name} {...props} />
}

Number5SquareSolid.prototype.name = name
