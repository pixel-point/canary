// This file is automatically generated. Modifying its content manually is discouraged.
import { registerIcon } from '@harnessio/svg-icon'
import type { IconProps } from '@harnessio/svg-icon-react'
import { Icon } from '@harnessio/svg-icon-react'

const name = 'multi-bubble-solid/noir'

registerIcon(
  name,
  `<svg width="24" height="24" fill="none" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" d="M1.25 16.5a6.25 6.25 0 1 1 3.374 5.55l-2.212.412a.75.75 0 0 1-.874-.874l.412-2.212a6.226 6.226 0 0 1-.7-2.876Z" clip-rule="evenodd"/><path fill="currentColor" fill-rule="evenodd" d="M22.75 10A8.75 8.75 0 0 0 14 1.25c-4.266 0-8.098 3.25-8.662 7.509l-.002.004v-.008.008l-.006.063a7.854 7.854 0 0 0-.02.238 7.75 7.75 0 0 1 9.628 9.622 10.375 10.375 0 0 0 .38-.037l.047-.006.014-.001.004-.001h.002l.017-.003a8.695 8.695 0 0 0 2.726-.922l3.335.621a.75.75 0 0 0 .874-.874l-.62-3.335A8.714 8.714 0 0 0 22.75 10Z" clip-rule="evenodd"/></svg>`
)

export function MultiBubbleSolid(props: IconProps) {
  return <Icon name={name} {...props} />
}

MultiBubbleSolid.prototype.name = name
