    ...
    <Dialog.Root open={open} onOpenChange={onClose}>
      <Dialog.Content className="max-w-xl">
        ...
        <div className="flex flex-col gap-y-7">
          <span>
            {isAdmin
              ? t('views:userManagement.removeAdmin.message', 'This will remove the admin tag for "{{name}}"', {
                  name: user?.display_name
                })
              : t('views:userManagement.grantAdmin.message', 'This will grant admin privileges to "{{name}}"', {
                  name: user?.display_name
                })}
          </span>
