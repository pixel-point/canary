...
import { cn, ILabelType, LabelValuesType, TranslationStore } from '@/views'
    ...
    <Table.Root tableClassName="table-fixed" variant="asStackedList">
      <Table.Header>
        <Table.Row>
          ...
          <Table.Head className="w-[54px]"></Table.Head>
        </Table.Row>
      </Table.Header>
        ...
        {labels.map(label => (
          <Table.Row key={label.id}>
            ...
            <Table.Cell className={cn('w-1/2 !py-3', { 'w-5/12': isSmallWidth })}>
              <span className="line-clamp-3 break-words text-sm text-foreground-3">{label?.description || ''}</span>
            </Table.Cell>
