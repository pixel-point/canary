...
const Dropdown = ({ children, className, open, onOpenChange }: PropsWithChildren<DialogProps>) => (
  <RadixDialog.Root open={open} onOpenChange={onOpenChange}>
    ...
    />
    <RadixDialog.Content className="absolute left-6 top-[58px] z-50 grid w-full max-w-[300px] gap-4 border bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 rounded-[10px] sm:rounded-lg">
...
>(({ className, ...props }, ref) => (
  <div className="flex items-center border-b pl-4 pr-4">
