...
interface ListFieldProps extends Omit<React.ComponentProps<"div">, "title">,
    VariantProps<typeof listFieldVariants> {
...
const List = ({
    ...
    <div
        className={cn("w-full [&>div:last-child]:border-0 border rounded-md w-full", className)}
        {...props}
    >{children}</div>
)
...
const ListField = ({
    ...
    <div
        className={cn(listFieldVariants({ right }), className)}
        {...props}
    >
        {title && <div className={cn("font-normal text-primary [&>em]:text-primary [&>em]:not-italic", label && "text-tertiary-background", className)}>{title}</div>}
        {description && <div className={cn("flex gap-2 text-tertiary-background whitespace-nowrap overflow-hidden text-ellipsis max-w-96", className)}>{description}</div>}
    </div>
)
